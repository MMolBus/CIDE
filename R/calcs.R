                     ...
                     if (manual.mask.test == T) {
                       ...
                       failed_thresholds <-
                         lapply(1:2, function(i)
                           list_threshold_results[[i]][is.na(list_threshold_results[[2]]) ==
                                                         T])
                       ...
                       surface_class <-
                         lapply(grep(paste(
                           succesfull_threshold_names, collapse = "|"
                         ), index.),
                         function(i)
                           paste0(as.integer(values(
                             calibration_results[[2]][[4]]
                           )),
                           as.integer(values(
                             list_threshold_results[[1]][[i]]
                           ))))
                       ...
                       failed_binary_surfaces <-
                         matrix(as.numeric(rep(NA, 4 * ncell(
                       ...
                       failed_binary_surfaces <-
                         lapply(1:length(failed_threshold_names), function(i)
                           failed_binary_surfaces)
                       ...
                       for (i in c(1:length(binary_surfaces))[index. %in% succesfull_threshold_names]) {
                         if (ncol(binary_surfaces[[i]]) != 4) {
                           cols <- c("surface.00",
                                     ...
                                     "surface.11")
                           missing_colnames <-
                             cols[is.element(cols, colnames(binary_surfaces[[i]])) != T]
                           missing_cols <-
                             matrix(0,
                                    ncell(list_threshold_results[[1]][[1]]),
                       ...
                       colnames <-
                         c(
                           ...
                           "y",
                           "index_value",
                           "predict.surface.class",
                           "baseline.surface.class",
                           "True.Negative",
                           "False.Positive",
                           "False.Negative",
                           "True.Positive"
                             ...
                             unname(do.call(c,
                                            lapply(c(1:length(index.)),
                                                   function(i)
                                                     unname(
                                                       c(
                                                         table(list.results[[1]][[i]][, 4])[2],
                                                         table(list.results[[1]][[i]][, 4])[1],
                                                         # table(list.results[[1]][[i]][,5])[1], table(list.results[[1]][[i]][,5])[2],
                                                         length(c(list.results[[1]][[i]][, 5])[c(list.results[[1]][[i]][, 5]) ==
                                                                                                 0]),
                                                         length(c(list.results[[1]][[i]][, 5])[c(list.results[[1]][[i]][, 5]) ==
                                                                                                 1]),
                                                         # table(list.results[[1]][[i]][,4])[2], table(list.results[[1]][[i]][,4])[1],
                                                   ...
                                                   function(i)
                                                     do.call(c,
                                                             lapply(0:1 , function(j)
                                                               descriptor.fun(list.results[[1]][[i]][, 3][list.results[[1]][[i]][, 4] == j],
                                                                              descriptors.))))))
                         } else{
                           int_surf_cover <-
                             unlist(lapply(c(1:length(index.)),
                       ...
                       plotpdf <-
                         function(lhist,
                           ...
                           pdfprint <-   function(hist, ind, man, over, name, asp) {
                             ...
                             plot(
                               man,
                               main =  paste(toupper(name)),
                               "moss values over whole scene",
                               axes = FALSE,
                               box = FALSE,
                               asp  = asp
                             )
