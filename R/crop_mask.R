...
crop_mask <-
  function(ras, msk, na.mask.value = 0) {
    ras2 <- ras
    for (i in 1:raster::nlayers(ras)){
      raster::values(ras[[i]])[raster::values(msk) == na.mask.value] <- NA
      ras2[[i]] <- raster::values(ras[[i]])
    }
    raster::values(msk)[raster::values(msk) == na.mask.value] <- NA
    ras2[[raster::nlayers(ras)+1]] <- msk


    ras3 <- raster::trim(ras2)
